...
WINDOW = Window(
    ...
    grid={'control': 'dw_1', 'dataobject': 'd_custbillwisercble', 'table': 'clients', 'keys': ['code'], 'computes': [{'name': 'address', 'expression': 'trim( clients_name ) + if(trim( clients_addr1 ) = ~"~",~"~",trim( clients_addr1 )+~",~") + if(trim( clients_addr2 ) = ~"~",~"~",trim( clients_addr2 )+~",~") + if(trim(  telephone ) = ~"~",~"~",\', Ph : \'+trim(  telephone )+~",~") ', 'format': '[general]', 'label': 'address', 'band': 'detail'}, {'name': 'totbal', 'expression': ' if(isnull(optsalesamt),0,optsalesamt) -  if(isnull(optrcvdamt ),0,optrcvdamt ) -  if(isnull(optsalesrcvdafter ),0,optsalesrcvdafter ) ', 'format': '########0.00', 'label': 'totbal', 'band': 'detail'}, {'name': 'mxduedate', 'expression': 'if( duedate > maxduedate,duedate,maxduedate)', 'format': 'dd/mm/yy', 'label': 'mxduedate', 'band': 'detail'}, {'name': 'compute_2', 'expression': ' sum( totbal for all)', 'format': '#########0.00', 'label': 'compute_2', 'band': 'summary'}, {'name': 'twgtbal', 'expression': 'sum(wgtbal for all)', 'format': '######0.000', 'label': 'twgtbal', 'band': 'summary'}], 'columns': [{'name': 'code', 'label': 'code', 'type': 'char'}, {'name': 'name', 'label': 'clients_name', 'type': 'char'}, {'name': 'addr1', 'label': 'clients_addr1', 'type': 'char'}, {'name': 'addr2', 'label': 'clients_addr2', 'type': 'char'}, {'name': 'ctype', 'label': 'ctype', 'type': 'char'}, {'name': 'telephone', 'label': 'telephone', 'type': 'char'}, {'name': 'minduedate', 'label': 'minduedate', 'type': 'date'}, {'name': 'maxduedate', 'label': 'maxduedate', 'type': 'date'}, {'name': 'cocode', 'label': 'cocode', 'type': 'char'}, {'name': 'duedate', 'label': 'duedate', 'type': 'date'}, {'name': 'optsalesamt', 'label': 'optsalesamt', 'type': 'decimal'}, {'name': 'optrcvdamt', 'label': 'optrcvdamt', 'type': 'decimal'}, {'name': 'optsalesrcvdafter', 'label': 'optsalesrcvdafter', 'type': 'decimal'}, {'name': 'wgtbal', 'label': 'wgtbal', 'type': 'decimal'}]},
    report={'dataobject': 'd_custbillwisercble', 'sql': 'SELECT clients.code AS clients_code, clients.name AS clients_name, clients.addr1 AS clients_addr1, clients.addr2 AS clients_addr2, clients.ctype AS clients_ctype, clients.telephone AS clients_telephone, clients.cocode AS clients_cocode, clients.duedate AS clients_duedate, (select min(salesm.duedate) from salesm where salesm.custcode = clients.code and salesm.tdate <= :rdate and salesm.control <= :rlevel and salesm.bal > 0) as minduedate, (select max(salesm.duedate) from salesm where salesm.custcode = clients.code and salesm.tdate <= :rdate and salesm.control <= :rlevel and salesm.bal > 0 ) as maxduedate, (select sum(salesm.netamt) from salesm where salesm.custcode = clients.code and salesm.tdate <= :rdate and salesm.control <= :rlevel ) as optsalesamt, (select sum(salesm.ramt) from salesm where salesm.custcode = clients.code and salesm.tdate <= :rdate and salesm.control <= :rlevel ) as optrcvdamt, (select sum(collection.tranamt + collection.discount) from collection,salesm where collection.islno = salesm.slno and salesm.custcode = clients.code and salesm.tdate <= :rdate and collection.tdate <= :rdate3 and salesm.control <= :rlevel ) as optsalesrcvdafter, (select sum(salesm.balwgt) from salesm where salesm.custcode = clients.code and salesm.tdate <= :rdate and salesm.control <= :rlevel ) as wgtbal FROM clients ORDER BY clients.name ASC, clients.code ASC', 'computes': [{'name': 'address', 'expression': 'trim( clients_name ) + if(trim( clients_addr1 ) = ~"~",~"~",trim( clients_addr1 )+~",~") + if(trim( clients_addr2 ) = ~"~",~"~",trim( clients_addr2 )+~",~") + if(trim(  telephone ) = ~"~",~"~",\', Ph : \'+trim(  telephone )+~",~") ', 'format': '[general]', 'label': 'address', 'band': 'detail'}, {'name': 'totbal', 'expression': ' if(isnull(optsalesamt),0,optsalesamt) -  if(isnull(optrcvdamt ),0,optrcvdamt ) -  if(isnull(optsalesrcvdafter ),0,optsalesrcvdafter ) ', 'format': '########0.00', 'label': 'totbal', 'band': 'detail'}, {'name': 'mxduedate', 'expression': 'if( duedate > maxduedate,duedate,maxduedate)', 'format': 'dd/mm/yy', 'label': 'mxduedate', 'band': 'detail'}, {'name': 'compute_2', 'expression': ' sum( totbal for all)', 'format': '#########0.00', 'label': 'compute_2', 'band': 'summary'}, {'name': 'twgtbal', 'expression': 'sum(wgtbal for all)', 'format': '######0.000', 'label': 'twgtbal', 'band': 'summary'}], 'args': ['rdate', 'rdate2', 'rlevel', 'rdate3'], 'arg_types': {'rdate': 'date', 'rdate2': 'date', 'rlevel': 'number', 'rdate3': 'date'}, 'tables': ['clients'], 'columns': [{'name': 'code', 'label': 'code', 'type': 'char'}, {'name': 'clients_name', 'label': 'clients_name', 'type': 'char'}, {'name': 'clients_addr1', 'label': 'clients_addr1', 'type': 'char'}, {'name': 'clients_addr2', 'label': 'clients_addr2', 'type': 'char'}, {'name': 'ctype', 'label': 'ctype', 'type': 'char'}, {'name': 'telephone', 'label': 'telephone', 'type': 'char'}, {'name': 'minduedate', 'label': 'minduedate', 'type': 'date'}, {'name': 'maxduedate', 'label': 'maxduedate', 'type': 'date'}, {'name': 'cocode', 'label': 'cocode', 'type': 'char'}, {'name': 'duedate', 'label': 'duedate', 'type': 'date'}, {'name': 'optsalesamt', 'label': 'optsalesamt', 'type': 'decimal'}, {'name': 'optrcvdamt', 'label': 'optrcvdamt', 'type': 'decimal'}, {'name': 'optsalesrcvdafter', 'label': 'optsalesrcvdafter', 'type': 'decimal'}, {'name': 'wgtbal', 'label': 'wgtbal', 'type': 'decimal'}]},
