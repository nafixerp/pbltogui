...
WINDOW = Window(
    ...
    grid={'control': 'dw_dim', 'dataobject': 'd_depositors_intpost', 'table': 'clients', 'keys': ['code'], 'computes': [{'name': 'compute_1', 'expression': 'code', 'format': '[general]', 'label': 'compute_1', 'band': 'detail'}], 'columns': [{'name': 'code', 'label': 'Party', 'type': 'char'}, {'name': 'name', 'label': 'name', 'type': 'char'}, {'name': 'intamt', 'label': 'intamt', 'type': 'decimal'}, {'name': 'intwgt', 'label': 'intwgt', 'type': 'decimal'}, {'name': 'sel', 'label': 'sel', 'type': 'long'}]},
    report={'dataobject': 'd_depositors_intpost', 'sql': 'SELECT clients.code AS clients_code, clients.name AS clients_name, (select clientsgs.intamt from clientsgs where clientsgs.code = clients.code) as intamt, (select clientsgs.intwgt from clientsgs where clientsgs.code = clients.code) as intwgt, (1) as sel FROM clients WHERE intwgt > 0 OR intamt > 0 ORDER BY clients.name ASC', 'computes': [{'name': 'compute_1', 'expression': 'code', 'format': '[general]', 'label': 'compute_1', 'band': 'detail'}], 'args': ['rdate', 'rlevel'], 'arg_types': {'rdate': 'date', 'rlevel': 'number'}, 'tables': ['clients'], 'columns': [{'name': 'code', 'label': 'Party', 'type': 'char'}, {'name': 'name', 'label': 'name', 'type': 'char'}, {'name': 'intamt', 'label': 'intamt', 'type': 'decimal'}, {'name': 'intwgt', 'label': 'intwgt', 'type': 'decimal'}, {'name': 'sel', 'label': 'sel', 'type': 'long'}]},
    opens=['w_clientshelp'],
)
