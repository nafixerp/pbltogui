...
WINDOW = Window(
    ...
    grid={'control': 'dw_acsummary', 'dataobject': 'd_custbillwise', 'table': 'salesm', 'keys': ['slno'], 'computes': [{'name': 'texamt', 'expression': ' eamt  +  sretamt ', 'format': '######0.00', 'label': 'texamt', 'band': 'detail'}, {'name': 'totdisc', 'expression': 'discount + if(isnull(discaftamt),0,discaftamt)', 'format': '######0.00', 'label': 'totdisc', 'band': 'detail'}, {'name': 'tnetamt', 'expression': ' netamt -  if(isnull(discaftamt),0,discaftamt)', 'format': '######0.00', 'label': 'tnetamt', 'band': 'detail'}, {'name': 'balance', 'expression': 'netamt - ramt  -  if(isnull(rcvdaftamt),0,rcvdaftamt)  - if(isnull(discaftamt),0,discaftamt) ', 'format': '######0.00', 'label': 'balance', 'band': 'detail'}, {'name': 'compute_15', 'expression': 'sum(balwgt for all)', 'format': '#####0.000', 'label': 'compute_15', 'band': 'summary'}, {'name': 'compute_6', 'expression': 'sum(billamt for all)', 'format': '######0.00', 'label': 'compute_6', 'band': 'summary'}, {'name': 'compute_9', 'expression': 'sum(texamt for all)', 'format': '######0.00', 'label': 'compute_9', 'band': 'summary'}, {'name': 'compute_10', 'expression': 'sum(totdisc for all)', 'format': '#####0.00', 'label': 'compute_10', 'band': 'summary'}, {'name': 'compute_7', 'expression': 'sum(tnetamt  for all)', 'format': '######0.00', 'label': 'compute_7', 'band': 'summary'}, {'name': 'compute_11', 'expression': 'sum(ramt for all)', 'format': '######0.00', 'label': 'compute_11', 'band': 'summary'}, {'name': 'compute_12', 'expression': 'sum(rcvdaftamt for all)', 'format': '######0.00', 'label': 'compute_12', 'band': 'summary'}, {'name': 'compute_13', 'expression': 'sum(balance  for all)', 'format': '######0.00', 'label': 'compute_13', 'band': 'summary'}], 'columns': [{'name': 'tdate', 'label': 'tdate', 'type': 'date'}, {'name': 'billamt', 'label': 'billamt', 'type': 'decimal'}, {'name': 'staxamt', 'label': 'staxamt', 'type': 'decimal'}, {'name': 'discount', 'label': 'discount', 'type': 'decimal'}, {'name': 'eamt', 'label': 'eamt', 'type': 'decimal'}, {'name': 'sretamt', 'label': 'sretamt', 'type': 'decimal'}, {'name': 'ramt', 'label': 'ramt', 'type': 'decimal'}, {'name': 'ramtafter', 'label': 'ramtafter', 'type': 'decimal'}, {'name': 'duedate', 'label': 'duedate', 'type': 'date'}, {'name': 'billno', 'label': 'billno', 'type': 'char'}, {'name': 'slno', 'label': 'slno', 'type': 'decimal'}, {'name': 'custname', 'label': 'custname', 'type': 'char'}, {'name': 'round', 'label': 'round', 'type': 'decimal'}, {'name': 'netamt', 'label': 'netamt', 'type': 'decimal'}, {'name': 'rcvdaftamt', 'label': 'rcvdaftamt', 'type': 'decimal'}, {'name': 'discaftamt', 'label': 'discaftamt', 'type': 'decimal'}, {'name': 'balwgt', 'label': 'balwgt', 'type': 'decimal'}]},
    report={'dataobject': 'd_custbillwise', 'sql': 'SELECT salesm.tdate AS salesm_tdate, salesm.billamt AS salesm_billamt, salesm.staxamt AS salesm_staxamt, salesm.discount AS salesm_discount, salesm.eamt AS salesm_eamt, salesm.sretamt AS salesm_sretamt, salesm.ramt AS salesm_ramt, salesm.ramtafter AS salesm_ramtafter, salesm.duedate AS salesm_duedate, salesm.billno AS salesm_billno, salesm.slno AS salesm_slno, salesm.custname AS salesm_custname, salesm.round AS salesm_round, salesm.netamt AS salesm_netamt, salesm.balwgt AS salesm_balwgt, (select sum(collection.tranamt) from collection where collection.islno = salesm.slno) as rcvdaftamt, (select sum(collection.discount) from collection where collection.islno = salesm.slno) as discaftamt FROM salesm WHERE salesm.tdate between :rdate1 and :rdate2 AND salesm.custcode = :rcode AND salesm.control <= :rlevel ORDER BY salesm.tdate ASC, salesm.slno ASC', 'computes': [{'name': 'texamt', 'expression': ' eamt  +  sretamt ', 'format': '######0.00', 'label': 'texamt', 'band': 'detail'}, {'name': 'totdisc', 'expression': 'discount + if(isnull(discaftamt),0,discaftamt)', 'format': '######0.00', 'label': 'totdisc', 'band': 'detail'}, {'name': 'tnetamt', 'expression': ' netamt -  if(isnull(discaftamt),0,discaftamt)', 'format': '######0.00', 'label': 'tnetamt', 'band': 'detail'}, {'name': 'balance', 'expression': 'netamt - ramt  -  if(isnull(rcvdaftamt),0,rcvdaftamt)  - if(isnull(discaftamt),0,discaftamt) ', 'format': '######0.00', 'label': 'balance', 'band': 'detail'}, {'name': 'compute_15', 'expression': 'sum(balwgt for all)', 'format': '#####0.000', 'label': 'compute_15', 'band': 'summary'}, {'name': 'compute_6', 'expression': 'sum(billamt for all)', 'format': '######0.00', 'label': 'compute_6', 'band': 'summary'}, {'name': 'compute_9', 'expression': 'sum(texamt for all)', 'format': '######0.00', 'label': 'compute_9', 'band': 'summary'}, {'name': 'compute_10', 'expression': 'sum(totdisc for all)', 'format': '#####0.00', 'label': 'compute_10', 'band': 'summary'}, {'name': 'compute_7', 'expression': 'sum(tnetamt  for all)', 'format': '######0.00', 'label': 'compute_7', 'band': 'summary'}, {'name': 'compute_11', 'expression': 'sum(ramt for all)', 'format': '######0.00', 'label': 'compute_11', 'band': 'summary'}, {'name': 'compute_12', 'expression': 'sum(rcvdaftamt for all)', 'format': '######0.00', 'label': 'compute_12', 'band': 'summary'}, {'name': 'compute_13', 'expression': 'sum(balance  for all)', 'format': '######0.00', 'label': 'compute_13', 'band': 'summary'}], 'args': ['rdate1', 'rdate2', 'rlevel', 'rcode', 'racname', 'ropbal'], 'arg_types': {'rdate1': 'date', 'rdate2': 'date', 'rlevel': 'number', 'rcode': 'string', 'racname': 'string', 'ropbal': 'decimal'}, 'tables': ['salesm'], 'columns': [{'name': 'tdate', 'label': 'tdate', 'type': 'date'}, {'name': 'billamt', 'label': 'billamt', 'type': 'decimal'}, {'name': 'staxamt', 'label': 'staxamt', 'type': 'decimal'}, {'name': 'discount', 'label': 'discount', 'type': 'decimal'}, {'name': 'eamt', 'label': 'eamt', 'type': 'decimal'}, {'name': 'sretamt', 'label': 'sretamt', 'type': 'decimal'}, {'name': 'ramt', 'label': 'ramt', 'type': 'decimal'}, {'name': 'ramtafter', 'label': 'ramtafter', 'type': 'decimal'}, {'name': 'duedate', 'label': 'duedate', 'type': 'date'}, {'name': 'billno', 'label': 'billno', 'type': 'char'}, {'name': 'slno', 'label': 'slno', 'type': 'decimal'}, {'name': 'custname', 'label': 'custname', 'type': 'char'}, {'name': 'round', 'label': 'round', 'type': 'decimal'}, {'name': 'netamt', 'label': 'netamt', 'type': 'decimal'}, {'name': 'rcvdaftamt', 'label': 'rcvdaftamt', 'type': 'decimal'}, {'name': 'discaftamt', 'label': 'discaftamt', 'type': 'decimal'}, {'name': 'balwgt', 'label': 'balwgt', 'type': 'decimal'}]},
    opens=['w_clientshelp'],
)
