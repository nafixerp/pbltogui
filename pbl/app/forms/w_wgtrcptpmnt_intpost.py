...
WINDOW = Window(
    ...
    grid={'control': 'dw_dim', 'dataobject': 'd_wgtrcptpmnt_intpost', 'table': 'clients', 'keys': ['code'], 'computes': [{'name': 'compute_1', 'expression': 'code', 'format': '[general]', 'label': 'compute_1', 'band': 'detail'}], 'columns': [{'name': 'code', 'label': 'Party', 'type': 'char'}, {'name': 'name', 'label': 'name', 'type': 'char'}, {'name': 'tcolln', 'label': 'tcolln', 'type': 'decimal'}, {'name': 'intamt', 'label': 'intamt', 'type': 'decimal'}]},
    report={'dataobject': 'd_wgtrcptpmnt_intpost', 'sql': "SELECT clients.code AS clients_code, clients.name AS clients_name, (select sum(wgtrcptpmnt.weight) from wgtrcptpmnt where wgtrcptpmnt.pcode = clients.code and wgtrcptpmnt.ttype = 'R' and wgtrcptpmnt.tdate <= :rdate and wgtrcptpmnt.control <= :rlevel) as tcolln, 0.0001 as intamt FROM clients WHERE tcolln > 0 ORDER BY clients.name ASC", 'computes': [{'name': 'compute_1', 'expression': 'code', 'format': '[general]', 'label': 'compute_1', 'band': 'detail'}], 'args': ['rdate', 'rlevel'], 'arg_types': {'rdate': 'date', 'rlevel': 'number'}, 'tables': ['clients'], 'columns': [{'name': 'code', 'label': 'Party', 'type': 'char'}, {'name': 'name', 'label': 'name', 'type': 'char'}, {'name': 'tcolln', 'label': 'tcolln', 'type': 'decimal'}, {'name': 'intamt', 'label': 'intamt', 'type': 'decimal'}]},
    opens=['w_clientshelp'],
)
