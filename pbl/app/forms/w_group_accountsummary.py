...
WINDOW = Window(
    ...
    report={'dataobject': 'd_group_accountsummary_sundbcr', 'sql': 'SELECT accountm.accode AS accountm_accode, clients.name AS clients_name, clients.addr1 AS clients_addr1, clients.addr2 AS clients_addr2, clients.addr3 AS clients_addr3, clients.city AS clients_city, accountm.opbal AS accountm_opbal, accountm.opbalb AS accountm_opbalb, accountm.actype2 AS accountm_actype2, clients.telephone AS clients_telephone, clients.cocode AS clients_cocode, clients.grp AS clients_grp, accountm.name AS accountm_name, accountm.clbal AS accountm_clbal, accountm.amount AS accountm_amount, (select sum(daybook.amount) from daybook where accountm.accode = daybook.accode and daybook.control <= :rlevel and daybook.tdate >= :rdate and daybook.tdate <= :rdate2) as ttran FROM accountm, clients WHERE accountm.accode = clients.code AND accountm.control <= :rlevel ORDER BY clients.name ASC', 'computes': [{'name': 'topbal', 'expression': "if(  rgrp = 'SUNDB' and  accountm_clbal < 0, abs(  accountm_clbal ), if(rgrp = 'SUNCR' and  accountm_clbal > 0,abs(  accountm_clbal ),0))", 'format': '#######0.00', 'label': 'topbal', 'band': 'detail'}, {'name': 'opstatus', 'expression': "if(rgrp = 'SUNDB','(Db)','(Cr)')", 'format': '[general]', 'label': 'opstatus', 'band': 'detail'}, {'name': 'debit', 'expression': "if(rgrp = 'SUNCR' and -cttran > accountm_clbal , accountm_clbal,if(rgrp = 'SUNDB' and -cttran >  accountm_clbal and accountm_clbal > 0, -cttran - accountm_clbal, if(cttran < 0, abs(cttran ),0)))", 'format': '########0.00', 'label': 'debit', 'band': 'detail'}, {'name': 'credit', 'expression': "if(rgrp = 'SUNDB' and cttran > -accountm_clbal , -accountm_clbal,if(rgrp = 'SUNCR' and cttran >  -accountm_clbal and accountm_clbal < 0, cttran + accountm_clbal, if(cttran > 0, cttran ,0)))", 'format': '#######0.00', 'label': 'credit', 'band': 'detail'}, {'name': 'tbal', 'expression': "abs(if(opstatus = '(Db)', -topbal, topbal) - debit + credit)", 'format': '#######0.00', 'label': 'tbal', 'band': 'detail'}, {'name': 'gopbal', 'expression': 'sum( topbal for all )', 'format': '#######0.00', 'label': 'gopbal', 'band': 'summary'}, {'name': 'compute_5', 'expression': "if(rgrp = 'SUNDB','(Db)','(Cr)')", 'format': '[general]', 'label': 'compute_5', 'band': 'summary'}, {'name': 'gdebit', 'expression': 'sum(debit for all)', 'format': '#######0.00', 'label': 'gdebit', 'band': 'summary'}, {'name': 'gcredit', 'expression': 'sum(credit for all)', 'format': '#######0.00', 'label': 'gcredit', 'band': 'summary'}, {'name': 'compute_3', 'expression': 'abs(sum( tbal  for all))', 'format': '#######0.00', 'label': 'compute_3', 'band': 'summary'}, {'name': 'tranamt', 'expression': ' if(isnull( cttran),0, cttran)  ', 'format': '#######0.00', 'label': 'tranamt', 'band': 'detail'}, {'name': 'netbal', 'expression': ' accountm_clbal -  debit  +  credit ', 'format': '#######0.00', 'label': 'netbal', 'band': 'detail'}, {'name': 'address', 'expression': ' if(trim( clients_addr1 ) = ~"~",~"~",trim( clients_addr1 )+~",~") + if(trim( clients_addr2 ) = ~"~",~"~",trim( clients_addr2 )+~",~") + if(trim(  clients_addr3 ) = ~"~",~"~",trim(  clients_addr3 )+~",~") +  trim(  clients_city ) +  if(clients_telephone <> \'\',\'Ph : \',\'\')+trim(   clients_telephone ) ', 'format': '[general]', 'label': 'address', 'band': 'detail'}], 'args': ['rdate', 'rdate2', 'rlevel', 'rgrp', 'rgrpname'], 'arg_types': {'rdate': 'date', 'rdate2': 'date', 'rlevel': 'number', 'rgrp': 'string', 'rgrpname': 'string'}, 'tables': ['accountm', 'clients'], 'columns': [{'name': 'accountm_accode', 'label': 'accountm_accode', 'type': 'char'}, {'name': 'clients_name', 'label': 'clients_name', 'type': 'char'}, {'name': 'clients_addr1', 'label': 'clients_addr1', 'type': 'char'}, {'name': 'clients_addr2', 'label': 'clients_addr2', 'type': 'char'}, {'name': 'clients_addr3', 'label': 'clients_addr3', 'type': 'char'}, {'name': 'clients_city', 'label': 'clients_city', 'type': 'char'}, {'name': 'accountm_opbal', 'label': 'accountm_opbal', 'type': 'decimal'}, {'name': 'accountm_opbalb', 'label': 'accountm_opbalb', 'type': 'decimal'}, {'name': 'accountm_actype2', 'label': 'accountm_actype2', 'type': 'char'}, {'name': 'clients_telephone', 'label': 'clients_telephone', 'type': 'char'}, {'name': 'clients_cocode', 'label': 'clients_cocode', 'type': 'char'}, {'name': 'clients_grp', 'label': 'clients_grp', 'type': 'char'}, {'name': 'accountm_name', 'label': 'accountm_name', 'type': 'char'}, {'name': 'accountm_clbal', 'label': 'accountm_clbal', 'type': 'decimal'}, {'name': 'accountm_amount', 'label': 'accountm_amount', 'type': 'decimal'}, {'name': 'cttran', 'label': 'cttran', 'type': 'decimal'}]},
