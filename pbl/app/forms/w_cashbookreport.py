...
WINDOW = Window(
    ...
    report={'dataobject': 'd_cashbook', 'sql': 'SELECT daybook.slno AS daybook_slno, daybook.tdate AS daybook_tdate, daybook.accode AS daybook_accode, daybook.amount AS daybook_amount, daybookpart.vchno AS daybookpart_vchno, daybookpart.particular AS daybookpart_particular, daybookpart.ic AS daybookpart_ic, (select accountm.name from accountm where accountm.accode = daybook.opaccode ) as othacname, (0) as sel FROM daybook, daybookpart WHERE daybook.slno = daybookpart.slno AND daybook.control <= :rlevel AND daybook.tdate >= :rdate1 AND daybook.accode = :rcode AND daybook.tdate <= :rdate2 ORDER BY daybook.tdate ASC, daybook.slno ASC', 'computes': [{'name': 'debit', 'expression': 'if(  daybook_amount < 0,string( abs( daybook_amount),~"######0.00~"),~"~" )', 'format': '[general]', 'label': 'debit', 'band': 'detail'}, {'name': 'credit', 'expression': 'if( daybook_amount > 0 , string( daybook_amount,~"#######0.00~"),~"~"  )', 'format': '[general]', 'label': 'credit', 'band': 'detail'}, {'name': 'compute_5', 'expression': '-(ropbal + cumulativeSum(  daybook_amount  for all ))', 'format': '########0.00', 'label': 'compute_5', 'band': 'detail'}, {'name': 'tdebit', 'expression': 'sum(if(  daybook_amount < 0, abs( daybook_amount), 0 ) for all)', 'format': '#######0.00', 'label': 'tdebit', 'band': 'summary'}, {'name': 'tcredit', 'expression': 'sum(if( daybook_amount > 0 , daybook_amount, 0 ) for all)', 'format': '#######0.00', 'label': 'tcredit', 'band': 'summary'}, {'name': 'compute_4', 'expression': ' if( (tdebit - tcredit  ) > 0,string(abs(tdebit - tcredit  ),~"#######0.00~"),~"~") ', 'format': '', 'label': 'compute_4', 'band': 'summary'}, {'name': 'compute_3', 'expression': ' if( (tdebit - tcredit ) > 0,~"~",string(abs(tdebit - tcredit  ),~"#######0.00~") ) ', 'format': '', 'label': 'compute_3', 'band': 'summary'}, {'name': 'netdebit', 'expression': ' if( ropbal < 0,abs(tdebit -  ropbal ),tdebit )', 'format': '#######0.00', 'label': 'netdebit', 'band': 'summary'}, {'name': 'netcredit', 'expression': ' if(  ropbal  < 0,tcredit,abs(tcredit +  ropbal ) ) ', 'format': '#######0.00', 'label': 'netcredit', 'band': 'summary'}, {'name': 'clbaldebit', 'expression': ' if( (tdebit - tcredit - ropbal ) > 0,string(abs(tdebit - tcredit - ropbal ),~"#######0.00~"),~"~") ', 'format': '', 'label': 'clbaldebit', 'band': 'summary'}, {'name': 'clbalcredit', 'expression': ' if( (tdebit - tcredit - ropbal ) > 0,~"~",string(abs(tdebit - tcredit - ropbal ),~"#######0.00~") ) ', 'format': '', 'label': 'clbalcredit', 'band': 'summary'}], 'args': ['rdate1', 'rdate2', 'rcode', 'rlevel', 'ropbal'], 'arg_types': {'rdate1': 'date', 'rdate2': 'date', 'rcode': 'string', 'rlevel': 'number', 'ropbal': 'decimal'}, 'tables': ['daybook', 'daybookpart'], 'columns': [{'name': 'daybook_slno', 'label': 'daybook_slno', 'type': 'decimal'}, {'name': 'daybook_tdate', 'label': 'daybook_tdate', 'type': 'date'}, {'name': 'daybook_accode', 'label': 'daybook_accode', 'type': 'char'}, {'name': 'daybook_amount', 'label': 'daybook_amount', 'type': 'decimal'}, {'name': 'daybookpart_vchno', 'label': 'daybookpart_vchno', 'type': 'char'}, {'name': 'daybookpart_particular', 'label': 'daybookpart_particular', 'type': 'char'}, {'name': 'cothacname', 'label': 'cothacname', 'type': 'char'}, {'name': 'daybookpart_ic', 'label': 'daybookpart_ic', 'type': 'char'}, {'name': 'sel', 'label': 'sel', 'type': 'long'}]},
