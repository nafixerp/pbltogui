...
WINDOW = Window(
    ...
    report={'dataobject': 'd_wstgmast', 'sql': 'SELECT wstgtable.weight1 AS wstgtable_weight1, wstgtable.weight2 AS wstgtable_weight2, wstgtable.wastage AS wstgtable_wastage, wstgtable.perc AS wstgtable_perc FROM wstgtable', 'computes': [], 'args': ['rcode', 'rtype'], 'arg_types': {'rcode': 'string', 'rtype': 'string'}, 'tables': ['wstgtable'], 'columns': [{'name': 'weight1', 'label': 'From Wgt', 'type': 'decimal'}, {'name': 'weight2', 'label': 'To Wgt', 'type': 'decimal'}, {'name': 'wastage', 'label': 'Wastage', 'type': 'decimal'}, {'name': 'perc', 'label': '%', 'type': 'decimal'}]},
    opens=['w_itemhelp'],
)
