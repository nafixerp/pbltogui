...
WINDOW = Window(
    ...
    grid={'control': 'dw_1', 'dataobject': 'd_staff_log_report', 'table': 'staff_log', 'keys': ['idno', 'tdate', 'ttime'], 'columns': [{'name': 'scode', 'label': 'scode', 'type': 'char'}, {'name': 'idno', 'label': 'idno', 'type': 'char'}, {'name': 'tdate', 'label': 'tdate', 'type': 'date'}, {'name': 'ttime', 'label': 'ttime', 'type': 'time'}, {'name': 'status', 'label': 'status', 'type': 'char'}, {'name': 'name', 'label': 'name', 'type': 'char'}]},
    report={'dataobject': 'd_staff_log_report', 'sql': 'SELECT staff_log.scode AS staff_log_scode, staff_log.idno AS staff_log_idno, staff_log.tdate AS staff_log_tdate, staff_log.ttime AS staff_log_ttime, staff_log.status AS staff_log_status, (select clients.name from clients where clients.code = staff_log.scode) as name FROM staff_log ORDER BY staff_log.tdate ASC, staff_log.ttime ASC, staff_log.idno ASC', 'args': ['rlevel', 'rdate1', 'rdate2'], 'arg_types': {'rlevel': 'number', 'rdate1': 'date', 'rdate2': 'date'}, 'tables': ['staff_log'], 'columns': [{'name': 'scode', 'label': 'scode', 'type': 'char'}, {'name': 'idno', 'label': 'idno', 'type': 'char'}, {'name': 'tdate', 'label': 'tdate', 'type': 'date'}, {'name': 'ttime', 'label': 'ttime', 'type': 'time'}, {'name': 'status', 'label': 'status', 'type': 'char'}, {'name': 'name', 'label': 'name', 'type': 'char'}]},
)
