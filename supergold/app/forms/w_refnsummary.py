...
WINDOW = Window(
    ...
    report={'dataobject': 'd_refnsummary', 'sql': 'SELECT refineryd.code AS refineryd_code, items.name AS items_name, refineryd.issuedwgt AS refineryd_issuedwgt, refineryd.issuedqty AS refineryd_issuedqty, refineryd.rcvdwgt AS refineryd_rcvdwgt, refineryd.rcvdqty AS refineryd_rcvdqty, refineryd.bottlestk AS refineryd_bottlestk, refineryd.testpcs AS refineryd_testpcs, refinerym.slno AS refinerym_slno, refinerym.docno AS refinerym_docno, refinerym.tdate AS refinerym_tdate, refinerym.refcode AS refinerym_refcode, refinerym.tbottlestk AS refinerym_tbottlestk, refinerym.ttestpcs AS refinerym_ttestpcs, refinerym.charge AS refinerym_charge, refinerym.paidamt AS refinerym_paidamt, refinerym.testperc AS refinerym_testperc, refinerym.status AS refinerym_status, refinerym.ttime AS refinerym_ttime, refineryd.mudless AS refineryd_mudless, refineryd.issuedstwgt AS refineryd_issuedstwgt FROM items, refineryd, refinerym WHERE items.code = refineryd.code AND refineryd.slno = refinerym.slno AND refinerym.refcode = :rcode AND refinerym.control <= :rlevel AND refinerym.tdate between :rdate1 and :rdate2 ORDER BY refinerym.tdate ASC, refinerym.docno ASC, refinerym.slno ASC', 'computes': [{'name': 'c_depr_w_cp', 'expression': 'if( refinerym_status = 2, refineryd_issuedwgt,0)   -  refineryd_rcvdwgt', 'format': '####0.000', 'label': 'c_depr_w_cp', 'band': 'detail'}, {'name': 'c_depr_wo_cp', 'expression': "if( refinerym_status = 2 and refineryd_code <> 'CP', refineryd_issuedwgt,0)   -  refineryd_rcvdwgt ", 'format': '####0.000', 'label': 'c_depr_wo_cp', 'band': 'detail'}, {'name': 'compute_16', 'expression': 'sum(  refinerym_paidamt for all distinct  refinerym_slno )', 'format': '#####0.00', 'label': 'compute_16', 'band': 'summary'}, {'name': 'c_totrcvd', 'expression': 'sum( refineryd_rcvdwgt for all)', 'format': '###0.000', 'label': 'c_totrcvd', 'band': 'summary'}, {'name': 'compute_13', 'expression': 'sum( refineryd_issuedqty for all)', 'format': '###0', 'label': 'compute_13', 'band': 'summary'}, {'name': 'c_totissued', 'expression': 'sum( refineryd_issuedwgt for all)', 'format': '###0.000', 'label': 'c_totissued', 'band': 'summary'}, {'name': 'c_totstwgt', 'expression': 'sum(refineryd_issuedstwgt for all)', 'format': '######0.000', 'label': 'c_totstwgt', 'band': 'summary'}, {'name': 'compute_15', 'expression': 'sum(refineryd_mudless for all)', 'format': '#####.000', 'label': 'compute_15', 'band': 'summary'}, {'name': 'compute_14', 'expression': 'sum( refineryd_bottlestk for all)', 'format': '##0.000', 'label': 'compute_14', 'band': 'summary'}, {'name': 'c_ttestpcs', 'expression': 'sum( refineryd_testpcs for all)', 'format': '##0.000', 'label': 'c_ttestpcs', 'band': 'summary'}, {'name': 'compute_18', 'expression': 'sum(  if(  refinerym_testperc >0 ,refinerym_testperc ,0) for all ) / sum(  if( refinerym_testperc > 0,1,0) for all ) ', 'format': '####.###', 'label': 'compute_18', 'band': 'summary'}, {'name': 'compute_12', 'expression': 'abs(rclweight + c_totissued - c_totrcvd)', 'format': '#####0.000', 'label': 'compute_12', 'band': 'summary'}, {'name': 'compute_11', 'expression': 'if((rclweight + c_totissued - c_totrcvd) > 0 ,~"To Receive~",~"To Give~")', 'format': '[general]', 'label': 'compute_11', 'band': 'summary'}, {'name': 'compute_6', 'expression': 'abs( rclbalance )', 'format': '######0.00', 'label': 'compute_6', 'band': 'summary'}, {'name': 'compute_7', 'expression': 'if(rclbalance < 0 ,~"To Receive~",~"To Give~")', 'format': '[general]', 'label': 'compute_7', 'band': 'summary'}, {'name': 'compute_10', 'expression': 'if(rclweight > 0 ,~"To Receive~",~"To Give~")', 'format': '[general]', 'label': 'compute_10', 'band': 'summary'}, {'name': 'c_netwgt', 'expression': 'abs( rclweight)', 'format': '#####0.000', 'label': 'c_netwgt', 'band': 'summary'}, {'name': 'compute_8', 'expression': ' rdepwgt ', 'format': '#####0.000', 'label': 'compute_8', 'band': 'summary'}, {'name': 'compute_9', 'expression': 'sum(c_depr_wo_cp for all)', 'format': '####0.000', 'label': 'compute_9', 'band': 'summary'}], 'args': ['rdate1', 'rdate2', 'rlevel', 'rcode', 'rname', 'rclbalance', 'rclweight', 'rdepwgt'], 'arg_types': {'rdate1': 'date', 'rdate2': 'date', 'rlevel': 'number', 'rcode': 'string', 'rname': 'string', 'rclbalance': 'decimal', 'rclweight': 'decimal', 'rdepwgt': 'decimal'}, 'tables': ['items', 'refineryd', 'refinerym'], 'columns': [{'name': 'refineryd_code', 'label': 'refineryd_code', 'type': 'char'}, {'name': 'items_name', 'label': 'items_name', 'type': 'char'}, {'name': 'refineryd_issuedwgt', 'label': 'refineryd_issuedwgt', 'type': 'decimal'}, {'name': 'refineryd_issuedqty', 'label': 'refineryd_issuedqty', 'type': 'long'}, {'name': 'refineryd_rcvdwgt', 'label': 'refineryd_rcvdwgt', 'type': 'decimal'}, {'name': 'refineryd_rcvdqty', 'label': 'refineryd_rcvdqty', 'type': 'long'}, {'name': 'refineryd_bottlestk', 'label': 'refineryd_bottlestk', 'type': 'decimal'}, {'name': 'refineryd_testpcs', 'label': 'refineryd_testpcs', 'type': 'decimal'}, {'name': 'refinerym_slno', 'label': 'refinerym_slno', 'type': 'decimal'}, {'name': 'refinerym_docno', 'label': 'refinerym_docno', 'type': 'char'}, {'name': 'refinerym_tdate', 'label': 'refinerym_tdate', 'type': 'date'}, {'name': 'refinerym_refcode', 'label': 'refinerym_refcode', 'type': 'char'}, {'name': 'refinerym_tbottlestk', 'label': 'refinerym_tbottlestk', 'type': 'decimal'}, {'name': 'refinerym_ttestpcs', 'label': 'refinerym_ttestpcs', 'type': 'decimal'}, {'name': 'refinerym_charge', 'label': 'refinerym_charge', 'type': 'decimal'}, {'name': 'refinerym_paidamt', 'label': 'refinerym_paidamt', 'type': 'decimal'}, {'name': 'refinerym_testperc', 'label': 'refinerym_testperc', 'type': 'decimal'}, {'name': 'refinerym_status', 'label': 'refinerym_status', 'type': 'long'}, {'name': 'refinerym_ttime', 'label': 'refinerym_ttime', 'type': 'time'}, {'name': 'refineryd_mudless', 'label': 'refineryd_mudless', 'type': 'decimal'}, {'name': 'refineryd_issuedstwgt', 'label': 'refineryd_issuedstwgt', 'type': 'decimal'}]},
    opens=['w_clientshelp'],
)
