...
WINDOW = Window(
    ...
    grid={'control': 'dw_1', 'dataobject': 'd_itemwiseprofit', 'table': 'items', 'keys': ['code'], 'computes': [{'name': 'costamt', 'expression': "if(  stkinnos = 'Y' ,  qtycostamt ,  wgtcostamt ) ", 'format': '#######0.00', 'label': 'costamt', 'band': 'detail'}, {'name': 'profit', 'expression': 'saleamt - costamt', 'format': '#######0.00', 'label': 'profit', 'band': 'detail'}, {'name': 'compute_5', 'expression': 'sum(tqty for all)', 'format': '#######0', 'label': 'compute_5', 'band': 'summary'}, {'name': 'compute_6', 'expression': 'sum(twgt for all)', 'format': '#######0.000', 'label': 'compute_6', 'band': 'summary'}, {'name': 'compute_1', 'expression': 'sum(saleamt for all)', 'format': '#######0.00', 'label': 'compute_1', 'band': 'summary'}, {'name': 'compute_2', 'expression': 'sum( costamt for all)', 'format': '#######0.00', 'label': 'compute_2', 'band': 'summary'}, {'name': 'compute_3', 'expression': 'sum( profit for all)', 'format': '#######0.00', 'label': 'compute_3', 'band': 'summary'}], 'columns': [{'name': 'code', 'label': 'Code', 'type': 'char'}, {'name': 'name', 'label': 'Name', 'type': 'char'}, {'name': 'saleamt', 'label': 'Sales Amt', 'type': 'decimal'}, {'name': 'stkinnos', 'label': 'stkinnos', 'type': 'char'}, {'name': 'qtycostamt', 'label': 'qtycostamt', 'type': 'decimal'}, {'name': 'wgtcostamt', 'label': 'wgtcostamt', 'type': 'decimal'}, {'name': 'tqty', 'label': 'tqty', 'type': 'long'}, {'name': 'twgt', 'label': 'twgt', 'type': 'decimal'}]},
    report={'dataobject': 'd_itemwiseprofit', 'sql': 'SELECT items.code AS items_code, items.name AS items_name, items.stkinnos AS items_stkinnos, (select sum(salesd.amount) from salesd,salesm where salesd.slno = salesm.slno and salesd.code = items.code and salesm.tdate >= :rdate1 and salesm.tdate <= :rdate2 and salesm.control <= :rlevel) as saleamt, (select sum(salesd.cost * salesd.qty) from salesd,salesm where salesd.slno = salesm.slno and salesd.code = items.code and salesm.tdate >= :rdate1 and salesm.tdate <= :rdate2 and salesm.control <= :rlevel) as qtycostamt, (select sum(salesd.cost * salesd.weight) from salesd,salesm where salesd.slno = salesm.slno and salesd.code = items.code and salesm.tdate >= :rdate1 and salesm.tdate <= :rdate2 and salesm.control <= :rlevel) as wgtcostamt, (select sum(salesd.qty) from salesd,salesm where salesd.slno = salesm.slno and salesd.code = items.code and salesm.tdate >= :rdate1 and salesm.tdate <= :rdate2 and salesm.control <= :rlevel) as tqty, (select sum(salesd.weight) from salesd,salesm where salesd.slno = salesm.slno and salesd.code = items.code and salesm.tdate >= :rdate1 and salesm.tdate <= :rdate2 and salesm.control <= :rlevel) as twgt FROM items WHERE saleamt > 0 ORDER BY items.name ASC', 'computes': [{'name': 'costamt', 'expression': "if(  stkinnos = 'Y' ,  qtycostamt ,  wgtcostamt ) ", 'format': '#######0.00', 'label': 'costamt', 'band': 'detail'}, {'name': 'profit', 'expression': 'saleamt - costamt', 'format': '#######0.00', 'label': 'profit', 'band': 'detail'}, {'name': 'compute_5', 'expression': 'sum(tqty for all)', 'format': '#######0', 'label': 'compute_5', 'band': 'summary'}, {'name': 'compute_6', 'expression': 'sum(twgt for all)', 'format': '#######0.000', 'label': 'compute_6', 'band': 'summary'}, {'name': 'compute_1', 'expression': 'sum(saleamt for all)', 'format': '#######0.00', 'label': 'compute_1', 'band': 'summary'}, {'name': 'compute_2', 'expression': 'sum( costamt for all)', 'format': '#######0.00', 'label': 'compute_2', 'band': 'summary'}, {'name': 'compute_3', 'expression': 'sum( profit for all)', 'format': '#######0.00', 'label': 'compute_3', 'band': 'summary'}], 'args': ['rdate1', 'rdate2', 'rlevel'], 'arg_types': {'rdate1': 'date', 'rdate2': 'date', 'rlevel': 'number'}, 'tables': ['items'], 'columns': [{'name': 'code', 'label': 'Code', 'type': 'char'}, {'name': 'name', 'label': 'Name', 'type': 'char'}, {'name': 'saleamt', 'label': 'Sales Amt', 'type': 'decimal'}, {'name': 'stkinnos', 'label': 'stkinnos', 'type': 'char'}, {'name': 'qtycostamt', 'label': 'qtycostamt', 'type': 'decimal'}, {'name': 'wgtcostamt', 'label': 'wgtcostamt', 'type': 'decimal'}, {'name': 'tqty', 'label': 'tqty', 'type': 'long'}, {'name': 'twgt', 'label': 'twgt', 'type': 'decimal'}]},
    opens=['w_itemhelp'],
)
