...
WINDOW = Window(
    ...
    report={'dataobject': 'd_pcard_print', 'sql': 'SELECT clients.code AS clients_code, clients.name AS clients_name, clients.addr1 AS clients_addr1 FROM clients WHERE clients.code = :rcode', 'computes': [{'name': 'bc', 'expression': "'*'+trim(code)+'*'", 'format': '[GENERAL]', 'label': 'bc', 'band': 'detail'}], 'args': ['rcode'], 'arg_types': {'rcode': 'string'}, 'tables': ['clients'], 'columns': [{'name': 'code', 'label': 'code', 'type': 'char'}, {'name': 'name', 'label': 'name', 'type': 'char'}, {'name': 'addr1', 'label': 'addr1', 'type': 'char'}]},
